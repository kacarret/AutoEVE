...
params = load_params('EVE\Data_storage/fine_tuned_params.json')
...
model.load_state_dict(torch.load('EVE\Data_storage/fine_tuned_model.pth', map_location=device), strict=False)

def generate_response(prompt, max_new_tokens=block_size):
    prompt_with_role = f"[User] {prompt}"
    context = encode_text(prompt_with_role)
    ...
    prompt_tokens = context[:, len(prompt_with_role):] # Remove prompt from context, however we are running into small issues with the output. Due to the discord api it has a ability to auto restart and reinitalze, this will not work in cmd but for now we will leave this. we almost have no choice.
