...
def encode_with_role(s, role):
    default_value = stoi.get('UNKOWN', -1)
    return [stoi[role]] + [stoi.get(c, default_value) for c in s]

pretrained_model_path = 'DataCollection/pretrained_model.pth'
pretrained_model_weights = torch.load(pretrained_model_path, map_location=device)
personality_dataset = 'DataCollection/new AI created dataset/Synth_data.txt'
...
def prune_model(model):
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            ...
            logging.info(f"Pruned layer: {name}")
        elif isinstance(module, nn.Embedding):
            prune.l1_unstructured(module, name='weight', amount=0.2)
            logging.info(f"Pruned layer: {name}")

    # Handling of model.blocks assumes that it is a sequential list of blocks
...
class Transformer(nn.Module):
    ...
    def generate(self, idx, max_new_tokens):
        for _ in tqdm(range(max_new_tokens)):
            ...
            if idx.size(1) >= 1024:  # For example, set a max length to avoid too long sequences
                break
        return idx
...
pretrained_dict = {k: v for k, v in pretrained_model_weights.items() if k in model_dict}
model_dict.update(pretrained_dict)
model.load_state_dict(model_dict)
nn.init.normal_(model.fc3.weight, mean=0.0, std=0.02)
if model.fc3.bias is not None:
    nn.init.zeros_(model.fc3.bias)
#we need to fix model pruning and i am assuming freeze is not working
#prune_model(model)
#freeze_layers(model, ['token_embedding_table', 'position_embedding_table'])
optimizer = AdamW(model.parameters(), lr=learning_rate, weight_decay=weight_decay)
scheduler = StepLR(optimizer, step_size=max_iters // num_updates, gamma=0.5)
early_stopping = EarlyStopping(patience=10, verbose=True)

def train_one_iteration(model, optimizer, scheduler, early_stopping):
    model.train()
    total_loss = 0
    for xb, yb in tqdm(data_loader, desc=f"Training", leave=False):
...
for iter in range(max_iters):
    with tqdm(total=1, desc=f"Evaluating iteration {iter}", leave=False) as pbar:
        if iter % eval_interval == 0 or iter == max_iters - 1:
            ...
            os.system('cls')  # This works on Windows; consider an alternative for cross-platform compatibility.
            ...
            logging.info(f"Step: {iter}, LR: {optimizer.param_groups[0]['lr']:.10f}")
            ...
            if early_stopping.best_model_wts is not None:
                torch.save(model, 'DataCollection/fine_tuned_model.pth')

        loss = train_one_iteration(model, optimizer, scheduler, early_stopping)
        pbar.update(1)
...
save_params(params, 'DataCollection/params1.json')
