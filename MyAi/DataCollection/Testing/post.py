...
scaler = GradScaler()
logging.basicConfig(filename='DataCollection/Testing/old_post_trainer.log', level=logging.INFO, format='%(asctime)s - %(levelname)s: %(message)s')

def clear_file(filename):
    open(filename, 'w').close()
clear_file('DataCollection\Testing\old_post_trainer.log')
        ...
        with tqdm(total=1, desc=f"Evaluating Iteration {iter}", leave=False) as pbar:
            ...
            if iter % eval_interval == 0 or iter == max_iters - 1:
                ...
                if early_stopping.best_model_wts is not None:
                    torch.save(early_stopping.best_model_wts, 'DataCollection/fine_tuned_model.pth')
...
save_params(params, 'DataCollection/fine_tuned_params.json')
