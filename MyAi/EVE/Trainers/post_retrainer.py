...
scaler = GradScaler()
logging.basicConfig(filename='DataCollection/Testing/post_retrainer.log', level=logging.INFO, format='%(asctime)s - %(levelname)s: %(message)s')

def clear_file(filename):
    open(filename, 'w').close()
clear_file('DataCollection\Testing\post_retrainer.log')
...
params = load_params('DataCollection/pre_retrainer_params.json')
...
past_model_path = 'DataCollection/pre_retrainer.pth'
...
def trainer():
            ...
            with tqdm(total=1, desc=f"Evaluating Iteration {iter}", leave=False) as pbar:
                ...
                if iter % eval_interval == 0 or iter == max_iters - 1:
                    ...
                    if early_stopping.best_model_wts is not None:
                        torch.save(early_stopping.best_model_wts, 'DataCollection/post_retrainer.pth')
    ...
    save_params(params, 'DataCollection/post_retrainer_params.json')
