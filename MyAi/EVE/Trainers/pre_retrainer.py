...
scaler = GradScaler()
logging.basicConfig(filename='DataCollection/Testing/pre_retrainer.log', level=logging.INFO, format='%(asctime)s - %(levelname)s: %(message)s')

def clear_file(filename):
    open(filename, 'w').close()
clear_file('DataCollection\Testing\pre_retrainer.log')
...
try:
    params = load_params('DataCollection/post_retrainer_params.json')
except FileNotFoundError:
    params = load_params('DataCollection/fine_tuned_params.json')
...
past_model_path = 'DataCollection/post_retrainer.pth'
pretrained_model_weights = torch.load(past_model_path, map_location=device)
data_path = 'DataCollection\input.txt'
...
def trainer():
            ...
            with tqdm(total=1, desc=f"Evaluating Iteration {iter}", leave=False) as pbar:
                ...
                if iter % eval_interval == 0 or iter == max_iters - 1:
                    ...
                    if early_stopping.best_model_wts is not None:
                        torch.save(early_stopping.best_model_wts, 'DataCollection/pre_retrainer.pth')
    ...
    save_params(params, 'DataCollection/pre_retrainer_params.json')

    return abs(early_stopping.best_score)
